...
_Last updated: 2026-08-24 — added Phase 4 (canonical `/new-in-bangkok` public routing + BNT storefront presentation, this session). Phase 3.5 (New in Bangkok canonical product update: Wednesday recurrence + ฿490 price) and Stage 10 Phase 3 (port minimum BNT public surface) are both COMPLETE and precede this entry. See "Stage 10" section below, which supersedes the "Stage 9 paused" section beneath it — that section is stale: Stage 9 was actually closed and merged to `main` (`8090708`) in a separate session after this doc's prior update. Read Stage 10 first, then Phase 4 immediately below it._
...
### Phases 5–9 (storefront-aware checkout/tickets/email, dynamic-pricing safety, Preview verification, domain-cutover plan) — NOT STARTED
...
Not built as of Stage 10 Phase 3 (explicitly out of scope that session, "THIS SESSION IS PHASE 3 ONLY"). Phase 4 (canonical `/new-in-bangkok` public routing + BNT storefront presentation) has since been completed in a later session — see the "Phase 4" section above this one for its full report. Phases 5–9 (storefront-aware checkout/tickets/email, dynamic-pricing safety, Preview verification, domain-cutover plan) remain not started.
