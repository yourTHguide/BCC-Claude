...
_Last updated: 2026-08-24 — added a Phase 7 follow-up entry (env-cleanup verification: Guide's manual Vercel deletions confirmed exact-match to the audit, the new generic Preview `CHECKOUT_DYNAMIC_PRICING` confirmed genuinely live after a fresh deploy, BCC regression clean, `STRIPE_PRICE_WEEKEND`/`WEEKDAY` confirmed still a live rollback dependency — keep). Original Phase 7 (Vercel environment-variable audit + BNT Preview QA), Phase 6 (BNT customer-journey + transactional-identity branding), Phase 5 (ticket pricing + storefront-aware booking/checkout), Phase 4 (canonical `/new-in-bangkok` public routing + BNT storefront presentation), Phase 3.5 (New in Bangkok canonical product update: Wednesday recurrence + ฿490 price), and Stage 10 Phase 3 (port minimum BNT public surface) are all COMPLETE and precede this entry. See "Stage 10" section below, which supersedes the "Stage 9 paused" section beneath it — that section is stale: Stage 9 was actually closed and merged to `main` (`8090708`) in a separate session after this doc's prior update. Read the Phase 7 follow-up first, then original Phase 7, then Phase 6, then Phase 5, then Stage 10, then Phase 4._
