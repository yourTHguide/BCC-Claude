...
_Last updated: 2026-08-24 — added Phase 6 (BNT customer-journey + transactional-identity branding, this session). Phase 5 (ticket pricing + storefront-aware booking/checkout), Phase 4 (canonical `/new-in-bangkok` public routing + BNT storefront presentation), Phase 3.5 (New in Bangkok canonical product update: Wednesday recurrence + ฿490 price), and Stage 10 Phase 3 (port minimum BNT public surface) are all COMPLETE and precede this entry. See "Stage 10" section below, which supersedes the "Stage 9 paused" section beneath it — that section is stale: Stage 9 was actually closed and merged to `main` (`8090708`) in a separate session after this doc's prior update. Read Phase 6 first, then Phase 5, then Stage 10, then Phase 4._
